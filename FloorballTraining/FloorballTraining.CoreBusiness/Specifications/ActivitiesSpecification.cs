...
public class ActivitiesSpecification : BaseSpecification<Activity>
{
    ...
    (string.IsNullOrEmpty(parameters.Tag) || x.ActivityTags.Any(t => t.Tag != null && parameters.Tag.ToLower().Split(";", StringSplitOptions.TrimEntries).Any(s => t.Tag.Name.ToLower() == s)))

    //(string.IsNullOrEmpty(parameters.Tag) || HasMatchingTag(x.ActivityTags, parameters.Tag))
