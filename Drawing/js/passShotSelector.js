...
function generatePassShotIconSvg(tool, width = 20, height = 20) {
    ...
    const headSize = Math.min((endX - startX) * 0.4, width * 0.3);
    ...
    if (tool.isDoubleLine) {
        const offset = strokeWidth * 1.5; // Space between double lines
        lines = `
...
export function updatePassShotTriggerDisplay(toolId) {
    ...
    if (dom.customPassShotSelectTrigger) {
        if (tool && tool.category === 'passShot') {
            ...
            dom.customPassShotSelectTrigger.innerHTML = `
                <span class="passShot-option-icon">${iconSvg}</span>
                <span>${tool.label}</span>
            `;
            dom.customPassShotSelectTrigger.dataset.value = toolId;
        } else {
            dom.customPassShotSelectTrigger.innerHTML = `<span>Select Pass/Shot...</span>`;
...
export function populateCustomPassShotSelector() {
    ...
    drawingTools.forEach(tool => {
        ...
        const iconSvg = generatePassShotIconSvg(tool);
        li.innerHTML = `<span class="passShot-option-icon">${iconSvg}</span><span>${tool.label}</span>`;
