...
import {
    ...
    MARKER_ARROW_RUN_ID, MARKER_ARROW_SHOT_LARGE_ID, // Use large shot marker ID
...
export function createCanvasElement(config, centerX, centerY) {
    ...
    if (availableContentWidth > 0 && availableContentHeight > 0 && config.svgContent) {
        if (config.type === 'activity') {
            try { /* ... activity svg handling ... */ } catch(e) { console.error("Error creating activity SVG container:", e); }
        } else if (config.type === 'library') {
            try { /* ... library foreignObject handling ... */ } catch(e) { console.error("Error creating library foreignObject:", e); }
        }
    }
...
export function createArrowElement(config, startX, startY, endX, endY) {
    ...
    const strokeColor = config.stroke || "black";
    const markerId = config.markerEndId; // Will be MARKER_ARROW_SHOT_LARGE_ID for shots

    if (config.isDoubleLine) {
        // --- Double Line Shot Arrow ---
        const angle = Math.atan2(endY - startY, endX - startX);
        const offset = strokeWidth * 1.5;
        const dx = Math.sin(angle) * offset / 2;
        const dy = -Math.cos(angle) * offset / 2;

        // Line 1 (Offset up/left)
        const line1 = document.createElementNS(SVG_NS, "line");
        line1.setAttribute("x1", String(startX + dx));
        line1.setAttribute("y1", String(startY + dy));
        line1.setAttribute("x2", String(endX + dx)); // End at original X+dx
        line1.setAttribute("y2", String(endY + dy)); // End at original Y+dy
        line1.setAttribute("stroke", strokeColor);
        line1.setAttribute("stroke-width", String(strokeWidth));
        group.appendChild(line1);

        // Line 2 (Offset down/right)
        const line2 = document.createElementNS(SVG_NS, "line");
        line2.setAttribute("x1", String(startX - dx));
        line2.setAttribute("y1", String(startY - dy));
        line2.setAttribute("x2", String(endX - dx)); // End at original X-dx
        line2.setAttribute("y2", String(endY - dy)); // End at original Y-dy
        line2.setAttribute("stroke", strokeColor);
        line2.setAttribute("stroke-width", String(strokeWidth));
        group.appendChild(line2);

        // Invisible Center Line for Marker Attachment
        const centerLine = document.createElementNS(SVG_NS, "line");
        centerLine.setAttribute("x1", String(startX));
        centerLine.setAttribute("y1", String(startY));
        centerLine.setAttribute("x2", String(endX)); // End at original X
        centerLine.setAttribute("y2", String(endY)); // End at original Y
        ...
        if (markerId) {
            centerLine.setAttribute("marker-end", `url(#${markerId})`);
        }
        group.appendChild(centerLine);

    } else {
        // --- Single Line Arrow (Pass, Run) ---
...
export function ensureHandles(element, currentWidth, currentHeight, isPlayer = false) {
    ...
    if (addMoveHandle) {
        if (width > 0 && height > 0 && hasBgRect) {
            const bgRect = element.querySelector('.element-bg'); const moveHandleWidth = Math.max(10, width * MOVE_HANDLE_WIDTH_PERCENT); const bgRectX = parseFloat(bgRect.getAttribute('x') || '0'); const bgRectY = parseFloat(bgRect.getAttribute('y') || '0'); const moveHandleX = bgRectX + (width - moveHandleWidth) / 2; const moveHandleY = bgRectY + MOVE_HANDLE_OFFSET;
            if (!moveHandle) { moveHandle = document.createElementNS(SVG_NS, "rect"); moveHandle.setAttribute("class", "move-handle"); if (bgRect.nextSibling) { element.insertBefore(moveHandle, bgRect.nextSibling); } else { element.appendChild(moveHandle); } }
            moveHandle.setAttribute("x", String(moveHandleX)); moveHandle.setAttribute("y", String(moveHandleY)); moveHandle.setAttribute("width", String(moveHandleWidth)); moveHandle.setAttribute("height", String(MOVE_HANDLE_HEIGHT));
        } else if (moveHandle) { moveHandle.remove(); }
    } else { moveHandle?.remove(); }
