...
import {
    ...
    DEFAULT_SHAPE_SIZE, DEFAULT_SHAPE_STROKE_WIDTH, DEFAULT_SHAPE_FILL_COLOR
} from './config.js';
...
export function createArrowElement(config, startX, startY, endX, endY) {
    ...
    if (config.isDoubleLine) {
        ...
        centerLine.setAttribute("x2", String(endX));
        centerLine.setAttribute("y2", String(endY));
        centerLine.setAttribute("stroke", "none");
        centerLine.setAttribute("fill", "none");
        centerLine.setAttribute("stroke-width", "1");
        centerLine.style.pointerEvents = "stroke";
        centerLine.style.strokeWidth = strokeWidth * 3;
        if (markerId) {
        ...
        line.style.strokeWidth = strokeWidth * 3;
...
export function createFreehandArrowElement(config, points) {
    ...
    path.style.strokeWidth = (config.strokeWidth || ARROW_STROKE_WIDTH_RUN) * 3;
    if (config.strokeDasharray) {
...
// --- Shape Element (Revised for Drawing) ---
export function createShapeElement(config, params) {
    // params: { x, y, width, height } for rect/square
    // params: { cx, cy, radius } for circle
    // config: Tool configuration (isFilled, shapeType, etc.)
    ...
    let fillColor = "none";
    if (isFilled) {
        fillColor = (appState.selectedColor.toLowerCase() === '#ffffff' || appState.selectedColor.toLowerCase() === 'white')
            ? DEFAULT_SHAPE_FILL_COLOR : appState.selectedColor;
    }
    const strokeWidth = DEFAULT_SHAPE_STROKE_WIDTH;

    let shapeSvg;
    let finalWidth, finalHeight, finalX, finalY; // Define variables
    ...
    switch (config.shapeType) {
        ...
        case 'triangle':
            ...
            const baseCenterX = finalX + finalWidth / 2;
            const point1 = `${baseCenterX},${topY}`;
            const point2 = `${finalX + finalWidth},${baseY}`;
            const point3 = `${finalX},${baseY}`;
            shapeSvg.setAttribute("points", `${point1} ${point2} ${point3}`);
            break;
        default:
    ...
    // No background rect needed as shape itself defines bounds and interactions

    // Set group transform to identity as coordinates are absolute now
    group.setAttribute("transform", "");
    group.dataset.rotation = "0";

    ensureHandles(group, finalWidth, finalHeight, false); // Handles relative to the shape itself
    makeElementInteractive(group);
    return group;
}
...
export function createBasicLineElement(config, startX, startY, endX, endY) {
    ...
    line.setAttribute("stroke-linecap", "round");
    line.style.pointerEvents = "stroke";
    line.style.strokeWidth = config.strokeWidth * 3 + 6; /* Increased click area */
    group.appendChild(line);
    const width = Math.abs(startX - endX);
    const height = Math.abs(startY - endY);
    ensureHandles(group, width, height, false);
...
// --- Utility: ensureHandles ---
export function ensureHandles(element, currentWidth, currentHeight, isPlayer = false) {
    if (!element) return;

    const transformList = element.transform.baseVal;
    const elementType = element.dataset.elementType;
    // For shapes/lines drawn directly, bgRect might not exist. Use the primary visual.
    ...
    } else { // Fallback if no visual found or no getBBox
        width = currentWidth ?? MIN_ELEMENT_WIDTH;
        height = currentHeight ?? MIN_ELEMENT_HEIGHT;
        relX = 0;
        relY = 0;
    ...
    const parentRotateCenter = {x: relX + width / 2, y: relY + height / 2};
    let parentRotateTransform = null;
    // Find existing rotate transform (usually last if it exists)
    ...
    if (allowRotation) {
        ...
        if (!parentRotateTransform) {
            parentRotateTransform = getOrAddTransform(transformList, SVGTransform.SVG_TRANSFORM_ROTATE, parentRotateCenter.x, parentRotateCenter.y);
        }
        ...
        // Remove rotation if not allowed and exists
    ...
    if (titleText) { /* ... existing title logic ... */
        ...
        const titleRotateCenterX = parentRotateCenter.x - titleX;
        const titleRotateCenterY = parentRotateCenter.y - titleY;
