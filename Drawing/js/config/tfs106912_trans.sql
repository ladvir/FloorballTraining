...
BEGIN
    ...
    WHERE OWNER = 'TMP_TABLE'
      AND UPPER(TABLE_NAME) LIKE UPPER('TFS106912_TRANS');
...
-- Backup tables
------------------------------------------------------------------
CREATE TABLE TMP_TABLE.TFS106912_TRANS
AS
SELECT C.ID,
       C.DOCTYPE,
       C.DOCCD,
...
DECLARE
    ...
    CURSOR C_TRANS IS SELECT * FROM TMP_TABLE.TFS106912_TRANS TMP;

    V_ROW C_TRANS%ROWTYPE;
BEGIN
    OPEN C_TRANS;

    LOOP
        FETCH C_TRANS INTO V_ROW;
