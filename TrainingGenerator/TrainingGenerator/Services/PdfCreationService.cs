        ...
        public string CreatePdf(Training training)
        {
            ...
            var table = document
                .AddSection().SetSize(PaperSize.A4).SetOrientation(PageOrientation.Portrait)
                    ...
                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsFlorballBallsNeeded)? "florbalové míčky": string.Empty);

                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsFlorballGateNeeded)? "florbalová branka": string.Empty);

                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsResulutionDressNeeded)? "rozlišovací dresy": string.Empty);

                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsConeNeeded)? "kužely": string.Empty);
                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsHurdleNeeded)? "skočky": string.Empty);
                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsJumpingLadderNeeded)? "skákací žebříky": string.Empty);
                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsJumpingRopeNeeded)? "švihadla": string.Empty);
                    accessories = string.Join(", ", trainingActivities.Any(ta=>ta.Activity.IsFootballBallNeeded)? "fotbalový míč": string.Empty);
            ...
            var row2 = table.AddRow().AddCell().AddTable()
                    ...
                    .AddRow().AddCellToRow("Pomůcky a vybavení").AddCell(accessories).ToTable();
