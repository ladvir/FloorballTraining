    ...
    ILogger<GamificationRecomputeJob> logger)
{
    ...
    {
        // Challenges first (#108/#156): their completions feed the *ChallengeReward XP the ledger derives next.
        var challengesCompleted = await challenges.RecomputeAllAsync(ct);
        var teamChallengesCompleted = await teamChallenges.RecomputeAllAsync(ct);
        ...
        if (xpInserted > 0 || badgesInserted > 0 || claimsInserted > 0 || challengesCompleted > 0 || teamChallengesCompleted > 0)
            logger.LogInformation(
                "Gamification recompute: +{Challenges} challenges, +{TeamChallenges} team challenges, +{Xp} XP events, +{Badges} badges, +{Claims} reward claims",
                challengesCompleted, teamChallengesCompleted, xpInserted, badgesInserted, claimsInserted);
    }
}
