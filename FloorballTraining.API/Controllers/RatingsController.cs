...
public class RatingsController(
    ...
    : BaseApiController
{
    /// <summary>A player may rate/edit/delete their own rating only within this many days after the event ends.</summary>
    private const int RatingWindowDays = 3;
    ...
    public async Task<IActionResult> Create([FromBody] AppointmentRatingDto dto)
    {
        ...
        if (DateTime.UtcNow > appointment.End.AddDays(RatingWindowDays))
            return BadRequest(new { message = $"Hodnocení lze zadat pouze do {RatingWindowDays} dnů od skončení události." });
    ...
    public async Task<IActionResult> Update(int id, [FromBody] AppointmentRatingDto dto)
    {
        ...
        if (!visEdit.CanModerate && rating.Appointment != null &&
            DateTime.UtcNow > rating.Appointment.End.AddDays(RatingWindowDays))
            return BadRequest(new { message = $"Hodnocení lze upravit pouze do {RatingWindowDays} dnů od skončení události." });
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        if (!visDel.CanModerate && rating.Appointment != null &&
            DateTime.UtcNow > rating.Appointment.End.AddDays(RatingWindowDays))
            return BadRequest(new { message = $"Hodnocení lze smazat pouze do {RatingWindowDays} dnů od skončení události." });
