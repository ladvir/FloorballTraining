...
type Action =
  ...
  | { type: 'setMatchDuration'; seconds: number }
  | { type: 'addField' }
...
function reducer(state: TournamentDto, action: Action): TournamentDto {
  switch (action.type) {
    ...
    case 'setMatchDuration':
      return { ...state, matchDurationSeconds: Math.max(0, action.seconds) }
    case 'addField':
        ...
        {setupOpen && (
          <div className="space-y-4 lg:col-span-4">
            ...
            <FormatPanel
              ...
              onChangeMatchDuration={(s) => dispatch({ type: 'setMatchDuration', seconds: s })}
            />
            <TasksPanel
...
// ── Per-match timer ─────────────────────────────────────────────────────────
// One shared AudioContext for every match's timer (each MatchTimer instance has its own countdown
// state, but they all play sound through this single context instead of creating one each).
...
function getAudioCtx(): AudioContext {
  ...
  // Browsers suspend a freshly-created context until resumed from a user gesture.
  if (sharedAudioCtx.state === 'suspended') void sharedAudioCtx.resume()
  return sharedAudioCtx
}

function beepSound(freq: number, duration: number) {
...
function sirenSound(duration = 2.4) {
  const ctx = getAudioCtx()
  const osc = ctx.createOscillator()
  const gain = ctx.createGain()
  osc.type = 'sawtooth'
  ...
  const legLen = 0.4
  for (let t = now; t < now + duration; t += 2 * legLen) {
    osc.frequency.setValueAtTime(600, t)
    osc.frequency.linearRampToValueAtTime(1100, t + legLen)
    osc.frequency.setValueAtTime(1100, t + legLen)
    osc.frequency.linearRampToValueAtTime(600, t + 2 * legLen)
  }
...
function useCountdownTimer(defaultSeconds: number) {
  // ponytail: reads defaultSeconds once at mount, not on every change — a tournament-settings edit
  // won't retroactively update an already-idle match's inputs. Add a sync effect if that's needed.
  const [minutesInput, setMinutesInput] = useState(() => Math.floor(defaultSeconds / 60))
  const [secondsInput, setSecondsInput] = useState(() => defaultSeconds % 60)
  const [remaining, setRemaining] = useState(minutesInput * 60 + secondsInput)
  const [status, setStatus] = useState<TimerStatus>('idle')
  // Wall-clock target instead of a per-second counter so drift/tab-throttling can't skew the countdown.
  const endAtRef = useRef<number | null>(null)
  const beepedSecondRef = useRef<number | null>(null)
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setRemaining(left)
      if (left <= TIMER_WARNING_SECONDS && left > 0 && beepedSecondRef.current !== left) {
        beepedSecondRef.current = left
        beepSound(880, 0.12)
      }
      if (left === 0) {
        setStatus('finished')
  ...
  function start() {
    getAudioCtx() // resume/create the shared AudioContext while still inside the click gesture
    const base = status === 'paused' ? remaining : minutesInput * 60 + secondsInput
    if (base <= 0) return
    endAtRef.current = Date.now() + base * 1000
    beepedSecondRef.current = null
    setRemaining(base)
    setStatus('running')
  }
  function pause() {
  ...
  function reset() {
    ...
    beepedSecondRef.current = null
    setRemaining(minutesInput * 60 + secondsInput)
  }
...
function MatchTimer({ defaultSeconds, onStart }: { defaultSeconds: number; onStart: () => void }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <TimerIcon className="h-3.5 w-3.5 flex-shrink-0 text-gray-400" />
      {timer.status === 'idle' ? (
        <div className="flex items-center gap-0.5">
          <input
            ...
            onChange={(e) =>
              timer.setMinutesInput(Math.max(0, Math.min(99, Number(e.target.value) || 0)))
            }
            title={t('tournaments.timerMinutesLabel')}
            className="h-6 w-8 rounded border border-gray-200 bg-white px-0.5 text-center text-xs focus:border-sky-500 focus:outline-none"
          />
          ...
          <input
            ...
            onChange={(e) =>
              timer.setSecondsInput(Math.max(0, Math.min(59, Number(e.target.value) || 0)))
            }
            title={t('tournaments.timerSecondsLabel')}
            className="h-6 w-8 rounded border border-gray-200 bg-white px-0.5 text-center text-xs focus:border-sky-500 focus:outline-none"
          />
        </div>
      ) : (
        <span
          title={isFinished ? t('tournaments.timerFinished') : undefined}
          className={`min-w-[2.5rem] text-center text-sm font-bold tabular-nums ${
            isFinished || isWarning ? 'animate-pulse text-red-600' : 'text-gray-800'
          }`}
        >
      ...
      {isRunning && (
        <button
          type="button"
          onClick={timer.pause}
          title={t('tournaments.timerPause')}
          className="inline-flex h-6 w-6 items-center justify-center rounded border border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100"
        >
      ...
      <button
        ...
        title={t('tournaments.timerReset')}
...
function FormatPanel({
  ...
  onChangeMatchDuration: (seconds: number) => void
}) {
  const { t } = useTranslation()
  const durationMinutes = Math.floor(matchDurationSeconds / 60)
  const durationSecondsPart = matchDurationSeconds % 60
  return (
    <Card>
      <CardContent className="space-y-3 py-4">
        ...
        <div className="border-t border-gray-100 pt-3">
          <p className="mb-1.5 text-xs font-medium text-gray-700">
            {t('tournaments.matchDurationLabel')}
          </p>
          <div className="flex items-center gap-1.5 text-sm">
            <input
              ...
              onChange={(e) => {
                ...
                onChangeMatchDuration(m * 60 + durationSecondsPart)
              }}
              className="h-9 w-14 rounded-lg border border-gray-300 bg-white px-2 text-center focus:border-sky-500 focus:outline-none"
            />
            <span className="text-gray-500">{t('tournaments.timerMinutesLabel')}</span>
            <input
              type="number"
              min={0}
              max={59}
              ...
              onChange={(e) => {
                ...
                onChangeMatchDuration(durationMinutes * 60 + s)
              }}
              className="h-9 w-14 rounded-lg border border-gray-300 bg-white px-2 text-center focus:border-sky-500 focus:outline-none"
            />
            <span className="text-gray-500">{t('tournaments.timerSecondsLabel')}</span>
          </div>
          <p className="mt-1.5 text-xs text-gray-500">{t('tournaments.matchDurationNote')}</p>
        </div>
      </CardContent>
    </Card>
  )
