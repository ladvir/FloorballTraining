    ...
    <div>
      ...
      {viewMode === 'list' ? (
        ...
        <CalendarView
          ...
          onDayClick={(date) => isAdmin && openCreate(date)}
        ...
        <div className="space-y-2">
          {appointments.map((apt, idx) => {
            ...
            return (
              <Card
                key={`${apt.id}-${idx}`}
                className={`${isAdmin ? 'cursor-pointer hover:border-sky-200 hover:shadow-md' : ''} transition-all ${isPast ? 'opacity-60' : ''}`}
                onClick={() => onEdit(apt)}
              >
          ...
          {days.map((day, i) => {
            ...
            return (
              <div
                ...
                } ${isAdmin ? 'cursor-pointer hover:bg-sky-50/30' : ''}`}
