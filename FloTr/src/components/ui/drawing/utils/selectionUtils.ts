 ...
 */
export function getSafeSelectedItems(
  items: Partial<SelectedItems> | null | undefined
): SelectedItems {
  const base = items ?? {}
  return {
...
export function selectItemsInRect(
  ...
  const select = (px: number, py: number) =>
    leftToRight ? checkInside(px, py) : checkTouching(px, py)

  return {
    players: items.players.map((p, i) => (select(p.x, p.y) ? i : -1)).filter((i) => i !== -1),
    equipment: items.equipment.map((e, i) => (select(e.x, e.y) ? i : -1)).filter((i) => i !== -1),
    lines: items.lines
      .map((l, i) => {
        if (leftToRight) {
    ...
    freehandLines: items.freehandLines
      ...
      .filter((i) => i !== -1),
    texts: items.texts.map((t, i) => (select(t.x, t.y) ? i : -1)).filter((i) => i !== -1),
    numbers: items.numbers.map((n, i) => (select(n.x, n.y) ? i : -1)).filter((i) => i !== -1),
    shapes: items.shapes
      .map((s, i) => {
