...
export function moveFreehandLines(
  ...
  return freehandLines.map((fl, i) => {
    ...
    const newPoints = orig.map((pt) =>
      movePoint(pt.x, pt.y, dx, dy, bounds.minX, bounds.minY, bounds.maxX, bounds.maxY)
    )
...
export function moveShapes(
  ...
  return shapes.map((s, i) => {
    ...
    if (s.type === 'triangle') {
      const newPoints = orig.points.map((pt) =>
        movePoint(pt.x, pt.y, dx, dy, bounds.minX, bounds.minY, bounds.maxX, bounds.maxY)
      )
