...
function tryParseDrawingState(json: string | undefined): SerializableDrawingState | null {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && 'fieldId' in parsed)
      return parsed as SerializableDrawingState
  } catch {
    /* not JSON */
  }
  return null
...
const DrawingComponentInner = ({
  ...
  const [selectedFieldId, setSelectedFieldId] = useState(
    restoredState?.fieldId ?? (svgXml ? 'null' : 'half-bottom')
  )
  ...
  const [activePlayerTool, setActivePlayerTool] = useState<(typeof playerTools)[number] | null>(
    null
  )
  ...
  const [activeSelectionTool, setActiveSelectionTool] = useState<null | (typeof selectionTools)[0]>(
    selectionTools[0]
  )
  ...
  useEffect(() => {
    if (activePlayerTool) lastPlayerToolRef.current = activePlayerTool
  }, [activePlayerTool])
  useEffect(() => {
    if (activeMovementTool) lastMovementToolRef.current = activeMovementTool
  }, [activeMovementTool])
  useEffect(() => {
    if (activeEquipmentTool) lastEquipmentToolRef.current = activeEquipmentTool
  }, [activeEquipmentTool])
  useEffect(() => {
    if (activeShapeTool) lastShapeToolRef.current = activeShapeTool
  }, [activeShapeTool])
  useEffect(() => {
    if (activeLineDrawConfig) lastLineDrawConfigRef.current = activeLineDrawConfig
  ...
  const [lastNumberClickPos, setLastNumberClickPos] = useState<{ x: number; y: number } | null>(
    null
  )
  ...
  useEffect(() => {
    trianglePointsRef.current = trianglePoints
  }, [trianglePoints])
  ...
  const [activityModalDrawingData, setActivityModalDrawingData] = useState<
    { stateJson: string; svgString: string } | undefined
  >(undefined)
  ...
  const legacySetSelectedItems = useCallback(
    (value: {
      players: number[]
      equipment: number[]
      lines: number[]
  ...
  const safeSetSelectedItems = useCallback(
    (value: SelectedItems | ((prev: SelectedItems) => SelectedItems)) => {
  ...
  const getCurrentDrawingState = useCallback(
    (): DrawingState => ({
      lines,
      freehandLines,
      players,
  ...
  const addFreehandLine = useCallback(
    (line: FreehandLine) => {
      saveHistory()
  ...
  const updateText = useCallback(
    (
      id: string,
      newText: string,
      style?: { fontSize?: number; fontWeight?: string; fontStyle?: string }
    ) => {
      saveHistory()
      setTexts((prev) =>
        prev.map((t) =>
  ...
  const handleMove = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      if (activeShapeTool && startPoint && activeShapeTool.toolId !== 'triangle') {
        ...
        const base: Omit<
          ShapeOnCanvas,
          'id' | 'type' | 'x' | 'y' | 'width' | 'height' | 'cx' | 'cy' | 'r' | 'points'
        > = {
          filled,
          strokeColor: '#1e3a5f',
          fillColor: 'rgba(30, 58, 95, 0.3)',
  ...
  const handleUp = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      if (activeShapeTool && activeShapeTool.toolId !== 'triangle' && startPoint) {
        ...
        const selected = selectItemsInRect(selectionRect, {
          players,
          equipment,
          lines,
          freehandLines,
  ...
  const handleSelect = useCallback(
    (
      type: 'player' | 'equipment' | 'line' | 'freehand' | 'text' | 'number' | 'shape',
      idx: number,
      e: React.MouseEvent
    ) => {
      e.stopPropagation()
  ...
  const handleDeleteSelected = useCallback(() => {
    ...
    if (selectedItems.freehandLines.length > 0) {
      setFreehandLines((prev) =>
        prev.filter((_, idx) => !selectedItems.freehandLines.includes(idx))
      )
    }
  ...
  const startEditExistingText = useCallback(
    (textItem: TextItem, e: React.MouseEvent) => {
      e.stopPropagation()
      textEditor.startEditing({
        id: textItem.id,
        x: textItem.x,
  ...
  return (
    <div id="drawing-component">
          ...
          >
            ...
            <g id="field-layer" pointerEvents="none">
              {selectedField && (
                <g
                  dangerouslySetInnerHTML={{
                    __html: getFieldOptionSvgMarkup(selectedField, FieldOptions),
                  }}
                />
              )}
            </g>
            <g id="content-layer">
              <FreehandLayer
                ...
                selectedItems={selectedItems.freehandLines}
                  ...
                  <path
                    ...
                    markerEnd={
                      activeDrawTool.arrow
                        ? `url(#arrow-${activeDrawTool.stroke.replace('#', '')})`
                        : undefined
                    }
                  />
                )}
                ...
                (() => {
                  ...
                  return (
                    <g pointerEvents="none" opacity={0.5}>
                      <line
                        x1={preview.x1 + nx * halfW}
                        y1={preview.y1 + ny * halfW}
                        x2={preview.x2 + nx * halfW}
                        y2={preview.y2 + ny * halfW}
                        stroke="#e6b800"
                        strokeWidth={2}
                      />
                      <line
              ...
              <EquipmentLayer
                equipment={equipment}
                selectedItems={selectedItems.equipment}
                handleSelect={handleSelect}
              />
              <TextLayer
                texts={texts}
                selectedItems={selectedItems.texts}
                handleSelect={handleSelect}
                onEditText={startEditExistingText}
              />
              <NumberSequenceLayer
                numbers={numbers}
                selectedItems={selectedItems.numbers}
                handleSelect={handleSelect}
              />
              <ShapeLayer
                shapes={shapes}
                selectedItems={selectedItems.shapes}
                handleSelect={handleSelect}
              />
              {shapePreview && shapePreview.type === 'circle' && (
                <circle
                  ...
                  cy={shapePreview.cy}
                  r={shapePreview.r}
                  fill={shapePreview.filled ? shapePreview.fillColor : 'none'}
                  stroke={shapePreview.strokeColor}
                  strokeWidth={2}
                  opacity={0.6}
                  pointerEvents="none"
                />
              ...
              {shapePreview &&
                (shapePreview.type === 'rectangle' || shapePreview.type === 'square') && (
                  <rect
                    x={shapePreview.x}
                    y={shapePreview.y}
        ...
        {textEditor.editingText &&
          (() => {
            ...
            if (svg && wrapperEl) {
              ...
              if (screen)
                panelPos = { x: screen.x - wr.left, y: Math.max(0, screen.y - wr.top - 40) }
            }
            return (
              <FloatingPanel
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 200 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    <button
                      type="button"
                      className={et.fontWeight === 'bold' ? 'selected' : ''}
                      onClick={() =>
                        textEditor.updateStyle({
                          fontWeight: et.fontWeight === 'bold' ? undefined : 'bold',
                        })
                      }
                    ...
                    <button
                      type="button"
                      className={et.fontStyle === 'italic' ? 'selected' : ''}
                      onClick={() =>
                        textEditor.updateStyle({
                          fontStyle: et.fontStyle === 'italic' ? undefined : 'italic',
                        })
                      }
        ...
        {activeNumberTool &&
          ...
          (() => {
            ...
            if (svg && wrapperEl) {
              ...
              const wr = wrapperEl.getBoundingClientRect()
              if (screen)
                panelPos = { x: screen.x - wr.left + 25, y: Math.max(0, screen.y - wr.top + 25) }
            }
            return (
              <FloatingPanel
              ...
              >
                ...
                <input
                  ...
                  style={{ width: 50, padding: '2px 4px', textAlign: 'center' }}
                />
                <button type="button" onClick={handleNumberReset} style={{ padding: '2px 6px' }}>
                  Reset
                </button>
              </FloatingPanel>
            )
        ...
        <ToolDropdown
          ...
          icon={
            <svg width={32} height={32}>
              <path
                d="M3,28 Q16,16 24,8"
                stroke={displayMovementTool?.stroke || '#000'}
                strokeWidth={1}
                strokeDasharray="4,2"
                fill="none"
                markerEnd="url(#arrow-000)"
              />
            </svg>
          }
          ...
          icon={
            <svg width={32} height={32} viewBox="0 0 32 32">
              <line
                ...
                strokeDasharray={
                  displayLineDrawConfig?.dash === 'dotted'
                    ? '2,4'
                    : displayLineDrawConfig?.dash === 'dashed'
                      ? '8,4'
                      : ''
                }
              />
              ...
              {displayShapeTool ? (
                displayShapeTool.toolId === 'circle' ? (
                  <circle
                    ...
                    cy="16"
                    r="12"
                    fill={displayShapeTool.filled ? 'rgba(30,58,95,0.3)' : 'none'}
                    stroke="#1e3a5f"
                    strokeWidth="2"
                  />
      ...
      <ActivitySearchModal
        ...
        onClose={() => {
          setActivityModalOpen(false)
          setActivityModalDrawingData(undefined)
        }}
