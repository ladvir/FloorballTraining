...
export type NumberSequenceTool = {
  toolId: 'number-seq'
  category: 'number'
  label: string
  fontSize: number
...
export const numberSequenceTools: NumberSequenceTool[] = [
  { toolId: 'number-seq', category: 'number', label: 'number-seq', fontSize: 18, color: '#000' },
]
    ...
    <div className="tool-group">
      {numberSequenceTools.map((tool) => (
        <div key={tool.toolId} className="tool-item">
          ...
          <span>{t('drawing.toolNumber')}</span>
        </div>
      ))}

      {activeNumberTool && (
        <div className="tool-item-inline" onClick={(e) => e.stopPropagation()}>
          <label title={t('drawing.startFrom')} htmlFor={'StartingNumber'} style={{ fontSize: 12 }}>
            {t('drawing.startFrom')}
          </label>
          <input
            id={'StartingNumber'}
            title={t('drawing.startFrom')}
            type="number"
            value={localValue}
